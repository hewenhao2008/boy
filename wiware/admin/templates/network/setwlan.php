...
<div class="container">
    <p class="heading">无线局域网(LAN)设置</p>
    <form action="actionWlan" method="post" class="form-wipark"  onSubmit="return checkform()">
        <p><a href="lannet" class="btn btn-lg btn-default btn-block">返回</a></p>
        <?php if (isset($flash['error']) ):?>
            <p class="text-danger"><?=$flash['error']?></p>
        <?php endif ?>
        <label class="control-label">盒子无线局域网IP：</label>
...
<script type="text/javascript">
    function isip(strIP) {
        ...
        if(re.test(strIP))
        {
            if( RegExp.$1==10 && RegExp.$2<256 && RegExp.$3<256 && RegExp.$4<256)
                return true;
        }
        return false;
    }
